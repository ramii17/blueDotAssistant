...
const generateDocumentHtml = (doc, approveUrl, rejectUrl) => {
    ...
    const accentColor = doc.docType === "QUOTE" ? "#3b82f6" : "#0d9488";
    ...
    const itemRows = doc.items.map(item => {
        ...
        return `
            <tr>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 14px; color: #1f2937;">${item.desc}</td>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 14px; text-align: center;">${item.qty}</td>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 14px; text-align: right;">${currencySymbol} ${item.price.toFixed(2)}</td>
        ...
        <div style="font-family: Arial, sans-serif; background: #ffffff; padding: 20px; border: 1px solid #e5e7eb; max-width: 600px; margin: auto; border-radius: 8px;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 25px; border-bottom: 1px solid #e5e7eb; padding-bottom: 15px;">
                <tr>
                    <td style="width: 50%;">
                        ...
                        <p style="margin: 0; font-size: 14px; color: #1f2937; font-weight: 600;">${doc.customerName}</p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 25px; border-collapse: collapse;">
                <thead>
                    <tr style="background: #f9fafb;">
                        ...
                        <th style="padding: 10px; text-align: center; font-size: 12px; color: #4b5563; text-transform: uppercase; width: 50px;">Qty</th>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td style="width: 50%; text-align: right;">
                        ...
                        <p style="font-size: 18px; color: #1f2937; margin: 15px 0 0 0; padding-top: 10px; border-top: 1px solid #d1d5db; font-weight: bold;">GRAND TOTAL: <span style="color: ${accentColor}; margin-left: 10px;">${currencySymbol} ${doc.total.toFixed(2)}</span></p>
            ...
            <div style="text-align: center; margin-top: 30px;">
                <p style="font-size: 14px; color: #4b5563; margin-bottom: 15px;">
                    Click a button below to officially register your decision:
                </p>
                <a href="${approveUrl}" style="padding:12px 20px; background:#16a34a; color:#fff; text-decoration:none; border-radius:6px; font-weight:bold; margin-right:15px; display: inline-block;">
                    ✅ Approve Quote
                </a>
                <a href="${rejectUrl}" style="padding:12px 20px; background:#dc2626; color:#fff; text-decoration:none; border-radius:6px; font-weight:bold; display: inline-block;">
                    ❌ Reject Quote
                </a>
                <p style="font-size: 12px; color: #6b7280; margin-top: 15px;">
                    For comments, questions, or to discuss, please simply **REPLY** to this email.
                </p>
            </div>
...
        const plainSummary = `${doc.docType} total: ${doc.currency} ${doc.total.toFixed(2)}. Please view the document and click the button to Approve/Reject.`;
...
// --- RESTORED: Client decision endpoint to log the status ---
